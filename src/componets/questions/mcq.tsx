            ...
            <div className="w-full pl-2">
              ...
              <ul className="w-full list-none p-0">
                {Array.from(question.answers.keys()).map((key:string)=>{
                  return(
                    <li key={key} className={`w-full cursor-pointer hover:border:2 hover:border-current pt-2 pb-2 text-medium text-black ${(question.answers.get(key)?.correct)?'border-2 border-sky-700':''}`}>
